...
vagrant ssh vm1 -c "docker build --rm -t glusterfs-server /docker_build/docker-glusterfs && \
                    docker stop glusterserver1 && docker rm glusterserver1"
vagrant ssh vm2 -c "docker build --rm -t glusterfs-server /docker_build/docker-glusterfs && \
...
docker run --privileged -d --name glusterserver1         \
	...
	glusterfs-server glusterserver2 "
...
docker run --privileged -d --name glusterserver2         \
	...
	glusterfs-server glusterserver1 start "
